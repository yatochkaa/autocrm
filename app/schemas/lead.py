...
from app.db.enums import LeadPriority, LeadSource, LeadStatus
...
class LeadRead(LeadBase):
    ...
    def analog_total(self) -> float:
        return round(
            sum(item.line_total for item in self.items if item.is_analog), 2
        )
...
# Совместимость с frontend Phase 2 и прежними импортами backend.
LeadOut = LeadRead
LeadStatusHistoryOut = StatusHistoryRead
