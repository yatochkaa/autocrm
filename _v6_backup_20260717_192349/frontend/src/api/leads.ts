...
export async function downloadLeadsExcel(filters: LeadFilters = {}): Promise<void> {
  const params: Record<string, string | number | boolean> = {}
  if (filters.status) params.status = filters.status
  if (filters.source) params.source = filters.source
  if (filters.manager_id) params.manager_id = filters.manager_id
  ...
  if (filters.search) params.search = filters.search
  if (filters.include_completed !== undefined) params.include_completed = filters.include_completed
  const response = await api.get('/leads/export.xlsx', { params, responseType: 'blob' })
  const url = URL.createObjectURL(response.data)
  const link = document.createElement('a'); link.href = url; link.download = 'autocrm-leads.xlsx'
